...
for i in range(train_identity):
    ...
    for j in range(data.shape[0]):
        labelData = labelss[j][1]  # (valence,arousal,dominance,liking)
        oneHot = []
        if labelData < 5:
            oneHot.append(1)
            oneHot.append(0)
...
with tf.Session() as sess:
    sess.run(init)
    train_writer = tf.summary.FileWriter("./logs/lstm_valence/train",sess.graph)
    test_writer = tf.summary.FileWriter("./logs/lstm_valence/test",sess.graph)
